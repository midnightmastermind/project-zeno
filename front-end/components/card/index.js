...
const Card = ({ blockId, date, content, deleteCard }) => {
  ...
  const forwardToPage = (id) => {
    router.push("/note/[blockid]", `/note/${id}`);
  };
  ...
  return (
    <div className={styles.cardWrapper}>
      ...
      {isContextMenuOpen && (
        <ContextMenu
          menuItems={[
            { id: "edit", label: "Edit", action: () => forwardToPage(blockId) },
            { id: "delete", label: "Delete", action: () => deletePage(blockId) },
          ]}
